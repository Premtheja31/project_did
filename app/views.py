...
def sub_cutoff(request):
    data=Student_marks.objects.all()
    if request.method=="POST":
        sub_name=request.POST["sub_name"].lower()
        cut_off=request.POST["cut_off"]
        new_data=[]
        for i in data:
            if i.sub_name>=int(cut_off):
                new_data.append(i)
        d={"data":data}
        return render(request,"display_percentage.html",d)
    return render(request,"sub_cutoff.html")
...
def all_cutoff(request):
    ...
    # print(data)
    # if request.method=="POST":
    #     cut_off=request.POST["cut"]
    #     print(cut_off)
    #     new_data=[]
    #     for i in data:
    #         if i.maths>=int(cut_off):
    #             if i.chemistry>=int(cut_off):
    #                 if i.physics>=int(cut_off):
    #                     if i.english>=int(cut_off):
    #                         if i.biology>=int(cut_off):
    #                             if i.history>=int(cut_off):
    #                                 if i.civics>=int(cut_off):
    #                                     if i.economics>=int(cut_off):
    #                                         new_data.append(i)
    #     d={"data":new_data}